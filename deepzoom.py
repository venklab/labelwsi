...
class DeepZoomGenerator:
    ...
    def __init__(self, osr, tile_size=254, overlap=1, limit_bounds=False):
        ...
        self.next_downsamples = tuple(
            int(self._osr.level_downsamples[slide_level + 1]
              / self._osr.level_downsamples[slide_level])
            for slide_level in range(self._osr.level_count - 1)
        )
    ...
    def get_tile(self, level, address):
        ...
        # (l0_location, slide_level, l_size)
        slide_level, l_size = args[1], args[2]
        if slide_level < (self._osr.level_count - 1):
            if z_size[0] * self.next_downsamples[slide_level] == l_size[0]:
                args[2] = z_size
                args[1] += 1
                print('debug: deepzoom.get_tile(), use next level')
